...
VMC.prototype.push = function(name, dir, manifest, callback) {
  ...
  this.appInfo(name, function(err, app){
    ...
    console.log('pushing ready to post');
    self.post(path, manifest, function(err, data){
      if (err) return callback(err);
      console.log('pushing ready to upload');
...
function VMC(target, user, pwd, token) {
  ...
  this.upload = function(dir, name, callback) {
    ...
    var prefix1 = '..';
    var prefix2 = 'fixtures';

    if(require('path').existsSync(zipFile))
      require('fs').unlinkSync(zipFile);

    console.log(zipFile);
    require('fs').readdirSync('.', function(err, files) { console.log(files); });

    console.log('ready to zip ' + dir + ' ' + zipFile);
    console.log('here again');
    vmcutils.zip(dir, prefix1 + zipFile, function(err, data){
      console.log('done zipping');
      if (err) return callback(err);
      var path = '/apps/' + name + '/application';
      self.uploadZip(prefix2 + zipFile, path, '[]', callback);
    });
  };
};
