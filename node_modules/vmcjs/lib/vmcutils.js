...
exports.zip = function(dir, zipFile, callback) {
  ...
  /*
  console.log('cd ' + dir + '; zip ' + (['-y', '-q', '-r', zipFile, '.', '-x', excludes].join(' ')) + '; cd ../..');  

  var child = exec('cd ' + dir + '; zip ' + (['-y', '-q', '-r', zipFile, '.', '-x', excludes].join(' ')) + '; cd ../../', function(err, stdout, stderr) { 
	  
	  if (err != null) return callback(new Error('Error during zip! '));
